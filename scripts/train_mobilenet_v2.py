"""
Training script for MobileNetV2 model.
...
def setup_logger(log_file):
    logger = logging.getLogger('mobilenet_v2')
...
def main():
    ...
    config_path = os.path.join(base_dir, 'configs', 'mobilenet_v2.yaml')
    ...
    model_name = config.get('model', {}).get('name', 'mobilenet_v2')
    ...
    # Replace mobilenet_v2 with mobilenetv2_100 which is common in timm, or just use config name
    # Using torchvision is another option, but we will stick to timm mapping or just use torchvision
    try:
        model = timm.create_model(model_name, pretrained=pretrained, num_classes=num_classes)
    except Exception as e:
        logger.warning(f"Failed to load via timm with name {model_name}. Attempting fallback name mobilenetv2_100")
